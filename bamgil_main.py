...
async def on_member_join(member):
    embed = discord.Embed(title="[길드 밤길 공지]",description='1: 서로 존중하는 채팅\n2: 주간보스 부탁 금지\n3: 일퀘를 함께 하는 분위기 권장\n4: 길뚫 부탁 강요X\n5: 길드 공지 미확인 및 길드 규칙 미숙지시 먼지등급으로 1주일간 강등 혹은 관리자와 의논 후 경고 및 추방\n6: 길드 장기 미접속(10일 이상)시 추방되며, 그 전에 쪽지로 사정을 말씀해주시면 보류\n7:디스코드 혹은 공지방중 하나는 필참\n8: 부캐길드: 밤비\n9: 오픈채팅:카카오톡-리부트2 밤길\n비밀번호:1213',color=0xFF0000)
...
async def on_message(message):
    if message.author.bot: #봇 일경우
        ...
        embed.add_field(name="밤길 봇", value="`!정보 닉네임`: 해당 유저의 레벨, 직업, 무릉, 마지막 접속일을 조회 \n`!이벤트`: 현제 진행중인 이벤트를 표시\n`!팅패`: 팅패 정보를 표시\n`!공지`: 길드 공지사항을 표시",inline=True)
        embed.set_footer(text="Made By 이나쁨",icon_url="https://i.imgur.com/Q8RTKGY.png")
        ...
        embed.set_image(url="https://i.imgur.com/UkpQWIy.png")
        embed.set_thumbnail(url="https://ssl.nx.com/s2/game/maplestory/renewal/common/logo.png")
        embed.set_footer(text="Made By 이나쁨",icon_url="https://i.imgur.com/Q8RTKGY.png")
        await message.channel.send(embed=embed) 
